...
create_users(){
    ...
    /usr/local/src/osdc_cloud_accounting/bin/sf_acct_create.py --nihfile $compiled_output_file --otherfile /etc/osdc_cloud_accounting/other_lists.csv --nocephquota --nocinderquota --multicloud  --ceph-keystone-s3  --debug
    fi
}
download_file
create_users
