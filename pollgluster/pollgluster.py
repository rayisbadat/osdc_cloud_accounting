...
class PollGluster:
    def __init__ (self,vol_name="USER-HOME", config_file=".settings"):
        ...
        self.gluster_cmd = [ "/usr/sbin/gluster", "volume", "quota", vol_name, "list"]
    ...
    def write_to_db(self):
        ...
        for path, value in self.dus.items():
            insert.append({'date': self.now_time, 'path': path, 'value': unitconverter.human2bytes(value)})
        conn.execute(self.du_table.insert(), insert)

    def get_average_du(self, start_date=None, end_date=None, path=None):
        """ Retrieve paths """
        if start_date is  None or end_date is None:
            sys.stderr.write("ERROR: Start and End Dates no specified in get_average_du")
            sys.exit(1)

        self.set_time_range(start_date=start_date, end_date=end_date)

        my_query = "SELECT AVG(value) FROM %s where ( date >= '%s' and date <= '%s' ) and path = '%s'" % ( self.settings['cloud'], self.start_time.strftime(self.settings['timeformat']), self.cieling_time.strftime(self.settings['timeformat']), path )
